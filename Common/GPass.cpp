...
#include "d3dApp.h"
#include "Window.h"
void PEPEngine::Common::GPass::AllocateDescriptors()
{
...
void PEPEngine::Common::GPass::InitRootSignature()
{
...
void PEPEngine::Common::GPass::InitPSO()
{
...
PEPEngine::Common::GPass::GPass(std::shared_ptr<GDevice> renderDevice): device(renderDevice)
{
	AllocateDescriptors();
...
PEPEngine::Graphics::GTexture& PEPEngine::Common::GPass::GetGTexture(DeferredPassRTVSlot slot)
{
	return gbuffers[slot];
}

PEPEngine::Graphics::GDescriptor& PEPEngine::Common::GPass::GetSRV()
{
	return deferredSRVDescriptor;
}

void PEPEngine::Common::GPass::Render(std::shared_ptr<GCommandList> cmdList)
{
	...
	for (int i = 0; i < GBufferMapsCount - 1; ++i)
	{
		cmdList->ClearRenderTarget(&deferredRTVDescriptor, i, DirectX::Colors::Black);
	}
	...
	cmdList->TransitionBarrier(gbuffers[DepthMap], D3D12_RESOURCE_STATE_DEPTH_READ);
	cmdList->FlushResourceBarriers();
}

void PEPEngine::Common::GPass::OnResize()
{
	...
	if (gbuffers.size() < GBufferMapsCount)
	{
		...
		gbuffers
			.push_back((GTexture(device, desc, L"Position GMAP", TextureUsage::RenderTarget)));
