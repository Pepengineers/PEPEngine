...
class AssetsLoader
{
	...
	custom_unordered_map<std::wstring, std::shared_ptr<GTexture>> textures = DXAllocator::CreateUnorderedMap<
		std::wstring, std::shared_ptr<GTexture>>();

	custom_unordered_map<std::wstring, std::shared_ptr<GShader>> shaders = DXAllocator::CreateUnorderedMap<
		std::wstring, std::shared_ptr<GShader>>();

	custom_unordered_map<std::wstring, std::shared_ptr<Material>> materials = DXAllocator::CreateUnorderedMap<
		std::wstring, std::shared_ptr<Material>>();

	custom_unordered_map<std::shared_ptr<Mesh>, std::shared_ptr<Material>> defaultMaterialForMeshFromFile =
		DXAllocator::CreateUnorderedMap<std::shared_ptr<Mesh>, std::shared_ptr<Material>>();
	...
	void CreateMaterialForMesh(std::shared_ptr<Mesh> mesh,
	                           const aiMaterial* material, std::shared_ptr<GCommandList> cmdList);
...
public:
	...
	std::shared_ptr<Model> GenerateSphere(std::shared_ptr<GCommandList> cmdList, float radius = 1.0f,
	                                      UINT sliceCount = 20, UINT stackCount = 20);

	std::shared_ptr<Model> GenerateQuad(std::shared_ptr<GCommandList> cmdList,
	                                    float depth = 0.0);
	std::shared_ptr<Model> GenerateTopQuad(std::shared_ptr<GCommandList> cmdList, float percent, float depth = 0.0);
	std::shared_ptr<Model> GenerateButtomQuad(std::shared_ptr<GCommandList> cmdList, float percent, float depth = 0.0);

	std::shared_ptr<Model> GetOrCreateModelFromFile(std::shared_ptr<DXLib::GCommandQueue> queue, std::string filePath);
