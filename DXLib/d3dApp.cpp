...
namespace DXLib
{
		...
		: appInstance(hInstance)
	{
		...
		{
			m_TearingSupported = Lazy<bool>([] { return CheckTearingSupport(); });
		}
	...
	{
		return m_TearingSupported.value();
	}
	...
	{
		return mainDevice->GetDescriptorHandleIncrementSize(type);
	}

	std::shared_ptr<GDevice> D3DApp::GetDevice()
	{
		return mainDevice;
	}

	void D3DApp::InitialDevices()
	{
		if (mainDevice == nullptr || devices.size() <= 0)
		{
			...
			while (dxgiFactory->EnumAdapters(i++, &adapter) != DXGI_ERROR_NOT_FOUND)
			{
				...
				{
					mainDevice = std::make_shared<GDevice>(adapter);
					devices.push_back(mainDevice);
					continue;
				}

				devices.push_back(std::make_shared<GDevice>(adapter));
			}
