...
void MultiSplitGPU::InitPipeLineResource()
{
	...
	for (int i = 0; i < GraphicAdapterCount; ++i)
	{
		...
		ambientPaths[i]->SetPSOs(*defaultPipelineResources[i].GetPSO(PsoType::Ssao),
		                         *defaultPipelineResources[i].GetPSO(PsoType::SsaoBlur));
...
void MultiSplitGPU::AddMultiDeviceOpaqueRenderComponent(GameObject* object, std::wstring modelName,
                                                        PsoType::Type psoType)
{
	for (int i = 0; i < GraphicAdapterCount; ++i)
	{
...
void MultiSplitGPU::PopulateShadowMapCommands(GraphicsAdapter adapterIndex, std::shared_ptr<GCommandList> cmdList)
{
	//Draw Shadow Map
	{
		...
		cmdList->SetRootShaderResourceView(StandardForwardShaderSlot::MaterialData,
		                                   *currentFrameResource->MaterialBuffers[adapterIndex]);
		cmdList->SetRootDescriptorTable(StandardForwardShaderSlot::TexturesMap, &srvTexturesMemory[adapterIndex]);
		...
		cmdList->SetRootConstantBufferView(StandardForwardShaderSlot::CameraData,
		                                   *currentFrameResource->PassConstantBuffers[adapterIndex], 1);
...
void MultiSplitGPU::PopulateForwardPathCommands(GraphicsAdapter adapterIndex, std::shared_ptr<GCommandList> cmdList)
{
	//Forward Path with SSAA
	{
		...
		cmdList->
			SetRootConstantBufferView(StandardForwardShaderSlot::CameraData,
			                          *currentFrameResource->PassConstantBuffers[adapterIndex]);

		cmdList->SetRootDescriptorTable(StandardForwardShaderSlot::ShadowMap, shadowPaths[adapterIndex]->GetSrvMemory());
		cmdList->SetRootDescriptorTable(StandardForwardShaderSlot::AmbientMap, ambientPaths[adapterIndex]->AmbientMapSrv(), 0);
		...
		case 1:
			{
				cmdList->SetRootDescriptorTable(StandardForwardShaderSlot::AmbientMap,
				                                shadowPaths[adapterIndex]->GetSrvMemory());
...
void MultiSplitGPU::PopulateDrawCommands(GraphicsAdapter adapterIndex, std::shared_ptr<GCommandList> cmdList,
                                         PsoType::Type type)
{
...
void MultiSplitGPU::PopulateDrawQuadCommand(GraphicsAdapter adapterIndex, std::shared_ptr<GCommandList> cmdList,
                                            GTexture& renderTarget, GMemory* rtvMemory, UINT offsetRTV)
{
	...
	cmdList->SetRootDescriptorTable(StandardForwardShaderSlot::AmbientMap, antiAliasingPaths[adapterIndex]->GetSRV());
...
void MultiSplitGPU::PopulateCopyResource(std::shared_ptr<GCommandList> cmdList, const GResource& srcResource,
...
void MultiSplitGPU::PopulateCopyResourceRegion(std::shared_ptr<GCommandList> cmdList, const GResource& srcResource,
                                               const GResource& dstResource)
{
