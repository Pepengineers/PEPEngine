...
protected:
	...
	void inline PopulateDrawCommands(GraphicsAdapter adapterIndex, std::shared_ptr<GCommandList> cmdList,
	                                 PsoType::Type type);
	void inline PopulateDrawQuadCommand(GraphicsAdapter adapterIndex, std::shared_ptr<GCommandList> cmdList,
	                                    GTexture& renderTarget, GMemory* rtvMemory, UINT
	                                    offsetRTV);
	void PopulateCopyResource(std::shared_ptr<GCommandList> cmdList, const GResource& srcResource,
	                          const GResource& dstResource);
	void inline PopulateCopyResourceRegion(std::shared_ptr<GCommandList> cmdList, const GResource& srcResource,
	                                       const GResource& dstResource);
...
private:
	...
	void inline AddMultiDeviceOpaqueRenderComponent(GameObject* object, std::wstring modelName,
	                                                PsoType::Type psoType = PsoType::Opaque);
